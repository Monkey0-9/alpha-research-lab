...
class SecurityMaster:
    ...
    def _bootstrap_sp500_master(self) -> None:
        ...
        seeds = [
            ...
            ("SEC-US-GOOGL-001", "GOOGL", "Alphabet Inc. Class A", "NASDAQ",
             [("GOOG", "2004-08-19", "2014-04-01"), ("GOOGL", "2014-04-02", None)]),
            ("SEC-US-AMZN-001", "AMZN", "Amazon.com Inc.", "NASDAQ",
             [("AMZN", "1997-05-15", None)]),
            ("SEC-US-NVDA-001", "NVDA", "NVIDIA Corporation", "NASDAQ",
             [("NVDA", "1999-01-22", None)]),
            ("SEC-US-META-001", "META", "Meta Platforms Inc.", "NASDAQ",
             [("FB", "2012-05-18", "2022-06-08"), ("META", "2022-06-09", None)]),
            ("SEC-US-BRKB-001", "BRK-B", "Berkshire Hathaway Inc. Class B", "NYSE",
             [("BRK.B", "1996-05-09", "2010-01-01"), ("BRK-B", "2010-01-02", None)]),
            ("SEC-US-TSLA-001", "TSLA", "Tesla Inc.", "NASDAQ",
             [("TSLA", "2010-06-29", None)]),
            ("SEC-US-JPM-001", "JPM", "JPMorgan Chase & Co.", "NYSE",
             [("JPM", "1969-03-05", None)]),
            ("SEC-US-V-001", "V", "Visa Inc.", "NYSE",
             [("V", "2008-03-19", None)]),
            ("SEC-US-UNH-001", "UNH", "UnitedHealth Group Inc.", "NYSE",
             [("UNH", "1984-10-18", None)]),
            ("SEC-US-XOM-001", "XOM", "Exxon Mobil Corporation", "NYSE",
             [("XOM", "1999-11-30", None)]),
            ("SEC-US-XRX-001", "XRX", "Xerox Holdings Corp (Delisted from S&P)",
             "NASDAQ", [("XRX", "1960-01-01", None)], False, "2021-03-22"),
        ]
    ...
    def adjust_prices(
        ...
        for act in actions:
            ...
            if act.action_type == ActionType.SPLIT and act.ratio > 0:
                ...
                mask = adj_df.index < act_date if not isinstance(
                    adj_df.index, pd.MultiIndex) else adj_df.index.get_level_values("date") < act_date
            ...
            elif (act.action_type == ActionType.DIVIDEND
                    and price_type == PriceType.TOTAL_RETURN
                    and act.cash_amount > 0):
                # Total return adjustment: price factor = (P_ex - Div) / P_ex
                mask = adj_df.index < act_date if not isinstance(
                    adj_df.index, pd.MultiIndex) else adj_df.index.get_level_values("date") < act_date
                ex_bar = adj_df.loc[adj_df.index >= act_date] if not isinstance(
                    adj_df.index, pd.MultiIndex) else adj_df.loc[adj_df.index.get_level_values("date") >= act_date]
